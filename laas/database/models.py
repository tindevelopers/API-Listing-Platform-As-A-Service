...
listing_categories = Table(
    'listing_categories',
    Base.metadata,
    Column('listing_id', UUID(as_uuid=True), ForeignKey('listings.id'), primary_key=True),
...
class User(Base):
    ...
    favorites = relationship("Listing", secondary=user_favorites, back_populates="favorited_by")
...
class Analytics(Base):
    ...
    # Constraints and indexes
    __table_args__ = (
        UniqueConstraint("tenant_id", "date", "metric_name", name="uq_analytics_tenant_date_metric"),
...
class ListingAnalytics(Base):
    ...
    # Constraints and indexes
    __table_args__ = (
        UniqueConstraint("listing_id", "date", "metric_name", name="uq_listing_analytics_listing_date_metric"),
