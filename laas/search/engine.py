...
from sqlalchemy import or_, func, and_, text, desc, asc
from sqlalchemy.dialects.postgresql import TSVECTOR

from laas.database.models import (
    Listing, Category, Tag, Media, Review, 
    ListingStatus, ReviewStatus
)
...
class SearchEngine:
    ...
    def search(
        ...
        # Category filtering
        if categories:
            q = q.join(Listing.categories).filter(
                Category.slug.in_(categories)
            )

        # Tag filtering
        if tags:
            q = q.join(Listing.tags).filter(
                Tag.slug.in_(tags)
            )
            ...
            if lat and lon:
                ...
                q = q.filter(
                    and_(
                        ...
                        text(f"ST_DWithin(ST_Point(listings.longitude, listings.latitude)::geography, ST_Point({lon}, {lat})::geography, {radius * 1609.344})")
                    )
                )
        ...
        if sort_by == "relevance" and query:
            ...
            if hasattr(Listing, 'search_vector'):
                q = q.order_by(desc(func.ts_rank(Listing.search_vector, func.plainto_tsquery('laas_search', query))))
            else:
            ...
            if lat and lon:
                q = q.order_by(
                    text(f"ST_Distance(ST_Point(listings.longitude, listings.latitude)::geography, ST_Point({lon}, {lat})::geography)")
                )
        elif sort_by == "rating":
            # Sort by average rating
            q = q.outerjoin(Review).filter(
                Review.status == ReviewStatus.APPROVED
            ).group_by(Listing.id).order_by(
    ...
    def get_facets(
        ...
        ).join(Listing.categories).filter(
            Category.tenant_id == tenant_id,
            Category.is_active == True
        ).group_by(Category.id, Category.name, Category.slug).all()

        # Get tag facets
        tag_facets = self.db.query(
            Tag.name,
            Tag.slug,
            func.count(Listing.id).label('count')
        ).join(Listing.tags).filter(
            Tag.tenant_id == tenant_id,
            Tag.is_active == True
        ).group_by(Tag.id, Tag.name, Tag.slug).all()
    ...
    def get_suggestions(
        ...
        title_suggestions = self.db.query(Listing.title).filter(
            Listing.tenant_id == tenant_id,
            Listing.is_public == True,
            Listing.status == ListingStatus.PUBLISHED,
            Listing.title.ilike(f"%{query}%")
        ...
        category_suggestions = self.db.query(Category.name).filter(
            Category.tenant_id == tenant_id,
            Category.is_active == True,
